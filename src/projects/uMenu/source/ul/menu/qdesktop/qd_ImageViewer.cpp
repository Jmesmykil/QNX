...
#include <ul/menu/qdesktop/qd_Theme.hpp>  // v2.6.0 — letterbox bg reads g_QdTheme.desktop_bg
...
void QdImageViewer::FreeInfoTex() {
    if (info_tex_ != nullptr) {
        // v2.0.3: info_tex_ comes from pu::ui::render::RenderText (line 248),
        // which is Plutonium-LRU-cache-owned.  Raw SDL_DestroyTexture bypasses
        // the cache contract and yields a P-B-class double-free when the LRU
        // later evicts the same handle — same incident class as B41/B42/B44/
        // B46/B52/B57/B61.  DeleteTexture honours the IsCacheOwnedTexture
        // gate and is a no-op on a cache-owned handle (the LRU reaps it).
        pu::ui::render::DeleteTexture(info_tex_);
        info_tex_ = nullptr;
    }
}
                              ...
                              const s32 origin_x, const s32 origin_y)
{
    ...
    // v2.6.0 — theme-aware: was hardcoded pure black.
    SDL_SetRenderDrawBlendMode(r, SDL_BLENDMODE_NONE);
    {
        const auto &db = ::ul::menu::qdesktop::g_QdTheme.desktop_bg;
        SDL_SetRenderDrawColor(r, db.r, db.g, db.b, 0xFFu);
    }
