...
// Returns AutoFolderIdx::None if the stable_id is not registered, if
// the matched bucket is currently disabled via SetAutoFolderEnabled(),
// or if the kind maps to None.
// The mapping rules:
...
// ── Per-category enable/disable (v2.0) ───────────────────────────────────────

/// Return whether the auto-folder bucket for idx is currently enabled.
/// Always returns false for AutoFolderIdx::None.
bool IsAutoFolderEnabled(AutoFolderIdx idx);

/// Enable or disable the auto-folder bucket for idx, then persist.
/// No-op for AutoFolderIdx::None.
void SetAutoFolderEnabled(AutoFolderIdx idx, bool enabled);

/// Load per-category enable flags from sdmc:/ulaunch/qos-folder-settings.toml.
/// Defaults all buckets to enabled if the file is absent or corrupt.
/// Called once at startup (from the same site as LoadCustomFolders).
void LoadAutoFolderSettings();

/// Persist current per-category enable flags to
/// sdmc:/ulaunch/qos-folder-settings.toml using atomic tmp+rename.
/// Called by SetAutoFolderEnabled automatically; also callable directly.
