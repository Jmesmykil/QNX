...
const char *QdPowerButtonElement::GlyphAssetName(Kind k) {
    switch (k) {
        ...
        case Kind::HekateUms: return "Hekate";  // reuses the same Hekate glyph PNG
...
// v2.0.3: tolerance bumped 20 → 30 px to match QdUserCardElement (the working
// reference).  Erista capacitive screen reports finger position with ~10-20 px
// jitter at the rest position; a 20-px tolerance was permanently clearing the
// press state mid-tap and forcing the user to retry.
static constexpr s32 POWER_CLICK_TOLERANCE_PX = 30;
                                   ...
                                   const pu::ui::TouchPoint touch_pos)
{
    ...
    // ── Touch state machine — edge-triggered like QdUserCardElement ────────────
    //
    // v2.0.3: the prior implementation re-evaluated `inside` AND `dist_sq > tol`
    // every active frame and cleared `press_inside_` permanently if either
    // failed for a single frame.  On the Erista capacitive screen, finger
    // jitter trips this on almost every gesture, so users have to tap multiple
    // times or hold long enough to luck into a clean frame at TouchUp.
    //
    // The fix is the same edge-triggered model QdUserCardElement uses
    // (`qd_UserCard.cpp:500-566`):
    //   • TouchDown   (was_touch_active_ false → true) records `down_x_/y_`
    //                 and sets `press_inside_` ONCE based on whether the down
    //                 point is inside the button rect.
    //   • TouchMove   updates `last_touch_x_/y_` only — never disturbs
    //                 `press_inside_` mid-gesture.
    //   • TouchUp     (was_touch_active_ true → false) checks final cumulative
    //                 drag distance against tolerance and fires on_click_ if
    //                 within bounds.
    ...
    if (touch_active_now) {
        ...
        if (!was_touch_active_) {
            ...
            const bool inside = (tx >= x_ && tx < x_ + BTN_W &&
                                 ty >= y_ && ty < y_ + BTN_H);
            ...
            // TouchMove — track position; do not flip press_inside_.
            last_touch_x_ = tx;
            last_touch_y_ = ty;
        }
    } else if (was_touch_active_) {
        // TouchUp — fire if press started inside and final drag distance is
        // within tolerance.  Cumulative drag (down_ → last_touch_), not per-
        // frame jitter.
        if (press_inside_) {
