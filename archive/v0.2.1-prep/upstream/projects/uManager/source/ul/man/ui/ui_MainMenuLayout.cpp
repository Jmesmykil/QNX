...
namespace ul::man::ui {
    ...
    void MainMenuLayout::ResetInfoText() {
        std::string info = "Q OS Manager v" UL_VERSION ", Q OS shell manager";
        if(g_MainApplication->IsAvailable()) {
            const auto ver = g_MainApplication->GetVersion();
            info += " | running Q OS v" + std::to_string((u32)ver.major) + "." + std::to_string((u32)ver.minor) + "." + std::to_string((u32)ver.micro);
