...
Q OS replaces HBMenu's NRO-launcher UI with the Vault file browser. Once Q OS
is installed, the standalone HBMenu surface is redundant and can be removed to
reclaim 6 to 17 MB and eliminate a competing launcher surface.
...
|---|---|---|
| Root NRO | `<sd>/hbmenu.nro` | 5 to 8 MB |
| Theme and language files | `<sd>/switch/hbmenu/` | 1 to 3 MB |
| Rare custom Album-applet override | `<sd>/atmosphere/contents/010000000000100B/exefs/main.nso` | 0 to 6 MB |

The script shows a dry-run preview of every file it will touch, plus the exact
...
- **`hbloader`** at `atmosphere/contents/010000000000100D/exefs/main.nso`.
  This is the NRO execution runtime. Without it, no `.nro` file runs at all,
  including the uManager Q OS installer NRO. It must remain.
...
- `du` for disk usage (standard, available on macOS and all Linux distributions)
- `curl` for `restore.sh` only, downloads the release zip from GitHub
- `unzip` for `restore.sh` only, unpacks the downloaded zip
