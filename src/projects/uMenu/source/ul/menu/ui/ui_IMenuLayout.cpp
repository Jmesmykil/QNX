    ...
    namespace {
        ...
        void OnFinishedSleep() {
            ...
            if(lockscreen_enabled) {
                // W12-FIX Bug 2: In QDESKTOP_MODE load the Q OS lockscreen
                // (QdLockscreenLayout) which renders wallpaper + clock and
                // requires no legacy PNG romfs assets.  The upstream
                // LockscreenMenuLayout (MenuType::Lockscreen) attempts to load
                // Connection/Battery PNGs that are absent from the Q OS romfs,
                // so it renders a blank screen — the regression seen beyond v3.2.4.
#ifdef QDESKTOP_MODE
                g_MenuApplication->LoadMenu(MenuType::QLockscreen, true);
#else
    ...
    void IMenuLayout::OnLayoutInput(const u64 keys_down, const u64 keys_up, const u64 keys_held, const pu::ui::TouchPoint touch_pos) {
        {
            ...
            if(!this->msg_queue.empty()) {
                ...
                switch(first_msg.msg) {
                    ...
                    // smi::MenuMessage::HomeLongRequest is intentionally NOT
                    // handled: HW test 2026-05-19 proved Nintendo's HOS-level
                    // long-press menu (power/airplane/capture options) always
                    // wins visually over any dialog we open in response.
                    // `AppletMessage_DetectLongPressingHomeButton` is a
                    // NOTIFICATION ("detected long-pressing"), not a hook to
                    // override the default behavior — the OS shows its menu
                    // unconditionally.  The protocol enum value + uSystem
                    // emit path stay (wire-compat; harmless), but uMenu
                    // doesn't try to compete with the HOS menu.
                    //
                    // Dev mini-menu is reachable via DOUBLE-TAP HOME instead
                    // (handled inside MainMenuLayout::OnHomeButtonPress) —
                    // that gesture is NOT HOS-reserved.
