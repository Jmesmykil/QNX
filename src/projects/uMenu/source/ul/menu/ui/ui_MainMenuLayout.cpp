...
#ifdef QDESKTOP_MODE
#include <ul/menu/qdesktop/qd_DevTools.hpp>  // v2.2.0 Phase 1: nxlink server auto-start
#endif
...
namespace ul::menu::ui {
    ...
    MainMenuLayout::MainMenuLayout() : IMenuLayout(), last_quick_menu_on(false), start_time_elapsed(false), is_incrementing_decrementing(false), next_reload_user_changed(false) {
...
#ifdef QDESKTOP_MODE
        {
            ...
            // Phase B: use the runtime global instead of baking DarkLiquidGlass() by
            // value.  g_QdTheme is initialised to DarkLiquidGlass() at startup and
            // mutated in-place by LoadThemeFromCache() before each RestartMenu().
            // Elements still snapshot the palette at construction time; a palette
            // swap takes effect on the next layout reload (RestartMenu).
            // W17-BUG2: set layout background to near-black so SDL_RenderClear
            // (which fires before wallpaper element OnRender) does not expose the
            // Plutonium DefaultBackgroundColor (light grey 0xE1E1E1) as a visible
            // stripe at the bottom edge of the screen.  0x0A0A14 matches the
            // Cold Plasma Cascade wallpaper base fill, making any sub-pixel gap
            // between the clear and the blit invisible regardless of scale rounding.
            this->SetBackgroundColor({ 0x0Au, 0x0Au, 0x14u, 0xFFu });
            ...
            // ── Window overlay (above status bar, below hot-corner) ──────
            // W10-BUG1B: windows must render AFTER the top-bar status elements
            // (date_text, battery_top_icon, etc.) so they occlude the bar
            // rather than rendering below it.  QdWindowOverlay calls
            // qdesktop_icons->RenderWindowLayer() which contains the same
            // window-render code that was previously inside
            // QdDesktopIconsElement::OnRender (now removed from there).
            this->qdesktop_window_overlay_ = qdesktop::QdWindowOverlay::New(this->qdesktop_icons);
            this->Add(this->qdesktop_window_overlay_);
            ...
            // v2.0.4.5: top-right hot-corner accent borders mirror the LEFT
            // overlay so the right hot zone has the same visual marker.
            this->qdesktop_right_overlay_ = qdesktop::QdHotCornerRightOverlay::New();
            this->Add(this->qdesktop_right_overlay_);
            ...
            // v2.2.0 Phase 1: nxlink inbound server is OFF by default and
            // user-toggled via the top-right hot-corner dropdown's "nxlink"
            // row.  Auto-start was tried in an interim build but caused
            // SDL_mixer audio skip during boot (socketInitializeDefault on
            // the main thread blocked the audio callback).  The dropdown
            // toggle calls dev::TryEnableNxlinkServer() / DisableNxlinkServer()
            // on user action, when the audio path is fully warmed up.
        }
    ...
    void MainMenuLayout::OnMenuUpdate() {
#ifdef QDESKTOP_MODE
        // W17-FIX1: drain deferred InitializeEntries result on the main thread.
        // The background thread set deferred_entries_ready_ (release) after writing
        // deferred_entries_.  We read it here with acquire semantics so we see the
        // fully-written vector.  Applied at most once per Initialize() call.
        if (!this->deferred_entries_applied_
                && this->deferred_entries_ready_.load(std::memory_order_acquire)) {
            this->deferred_entries_applied_ = true;
            if (this->qdesktop_icons) {
                UL_LOG_INFO("W17-FIX1: applying deferred entries (%zu) on main thread",
                            this->deferred_entries_.size());
                this->qdesktop_icons->SetApplicationEntries(this->deferred_entries_);
                this->qdesktop_icons->SetSpecialEntries(this->deferred_entries_);
            }
        }
        ...
        // v2.0.3: first-frame-of-Main guard for the welcomer auto-open.
        // QdDesktopIconsElement::OnRender already renders the welcomer via
        // RenderFirstBootWelcomeIfOpen(); the render call here was a v1.8.27
        // duplicate path that was never removed.  Drop it (single render path)
        // and use this hook only to fire the one-shot ShouldShow/Open.
        if (!first_main_frame_done_) {
            first_main_frame_done_ = true;
...
#ifdef QDESKTOP_MODE
    // W17-FIX1-REVERT: helper retained as dead code for the v3.6 boot-speed
    // refactor that will port InitializeEntries side-effects properly.  Marked
    // [[maybe_unused]] so -Werror=unused-function doesn't gate the build.
    [[maybe_unused]]
    // Calls the slow IPC path (InitializeEntries / EnsureApplicationRecordsAndViews)
    // then enriches entries from records.bin, stores in out_entries, and signals
    // the ready flag.  MUST NOT touch any UI objects — only writes to the vector
    // and then sets the atomic, both of which are consumed on the main thread in
    // OnMenuUpdate after deferred_entries_ready_ is observed true.
    static void RunDeferredInitializeEntries(std::vector<ul::menu::Entry> &out_entries,
                                             std::atomic<bool>             &ready_flag)
    {
        // Step 1: the slow ns:am IPC enumeration (~1547ms on a 64-app library).
        g_GlobalSettings.InitializeEntries();

        // Step 2: load SD .m.json entry files (fast filesystem read).
        std::vector<ul::menu::Entry> entries =
            ul::menu::LoadEntries(ul::menu::GetActiveMenuPath());
        UL_LOG_INFO("W17-FIX1 BG: LoadEntries returned %zu", entries.size());

        // Step 3: enrich / fallback from records.bin (written by uManager.nro
        // at full-privilege with NACP names from nsextGetApplicationControlData
        // CacheOnly).  In hbloader/applet mode the NS sysmodule denies direct
        // nsextGetApplicationControlData (rc=0x1F800), so this is also the
        // primary source in those contexts.
        {
            ...
            if (entries.empty() && rb_ok) {
                UL_LOG_INFO("W17-FIX1 BG: records.bin full-fallback yielded %zu",
                            rb_entries.size());
                ...
                for (const auto &rb : rb_entries) {
                    if (!rb.control.name.empty()) {
                        rb_name_map.emplace(rb.app_info.app_id, rb.control.name);
                    }
                }
                size_t enriched = 0;
                for (auto &e : entries) {
                ...
                UL_LOG_INFO("W17-FIX1 BG: records.bin enrichment: %zu/%zu",
                            enriched, entries.size());
            } else if (!rb_ok) {
                UL_LOG_INFO("W17-FIX1 BG: records.bin unavailable");
            }
        }

        // Step 4: write result and signal — main thread picks it up in OnMenuUpdate.
        out_entries = std::move(entries);
        ready_flag.store(true, std::memory_order_release);
        UL_LOG_INFO("W17-FIX1 BG: deferred entries ready (%zu entries)",
                    out_entries.size());
    }
#endif // QDESKTOP_MODE
    ...
    void MainMenuLayout::Initialize() {
        ...
        // v3.5.1 stable: InitializeEntries MUST run in both modes — qdesktop's
        // SetApplicationEntries / SetSpecialEntries depend on the upstream
        // entry vector being populated.
        //
        // The v3.5.0 W17-FIX1 attempt to defer this to a background thread
        // BROKE in two ways: (a) deferred path skipped the synchronous side
        // effects (g_ActiveMenuPath, g_ApplicationRecords) the renderer needs
        // for first paint → "black screen with buttons" regression; (b) when
        // the revert removed the bg-thread spawn but left the OnMenuUpdate
        // apply-on-ready hook, SetApplicationEntries / SetSpecialEntries
        // were NEVER called because the ready flag never flipped — Games
        // folder rendered empty.  Both issues fixed by restoring the proven
        // v3.4.0 synchronous path here.  Boot time costs ~1.5 s on this
        // path; v3.6 will port both the IPC loop AND the side effects
        // properly to a worker thread, not just one.
        //
        // HOME-FROM-APP BLACK-SCREEN FIX (2026-06-19):
        // When uMenu is relaunched in MainMenu mode while a title is still
        // being terminated by uSystem, ns:am may be in a transitional state
        // (closing the title's NACP/IPC session) and ListApplicationRecords()
        // can block for several seconds — keeping the screen black and
        // preventing the render loop (+ DebugServer) from starting.
        //
        // Guard: if suspended_app_id is non-zero (title not yet fully
        // terminated as reported by uSystem in the SystemStatus blob), skip
        // the blocking NS IPC here and rely on the records.bin fallback in
        // ApplyAppAndSpecialEntries() for the first desktop paint.
        // Reload() is called on every LoadMenu(Main) transition and will
        // call ApplyAppAndSpecialEntries() again once the title has fully
        // exited and the NS state is clean.
        //
        // Non-QDESKTOP_MODE builds: unchanged (upstream ulaunch path).
#ifdef QDESKTOP_MODE
        if (g_GlobalSettings.IsTitleSuspended()) {
            UL_LOG_INFO("MainMenuLayout::Initialize: title suspended "
                        "(app_id=0x%016lX) — skipping blocking NS InitializeEntries; "
                        "records.bin fallback will populate icons on first paint",
                        g_GlobalSettings.system_status.suspended_app_id);
            // Skip InitializeEntries() entirely.  ApplyAppAndSpecialEntries()
            // (called by Reload() below) uses records.bin as a full fallback
            // when entries.empty(), so icons will still populate on first paint
            // without any NS IPC.
        } else {
            g_GlobalSettings.InitializeEntries();
        }
...
#ifdef QDESKTOP_MODE
        // v3.5.1 stable: factored into a helper so Reload() can call the same
        // path after user-unlock.  Without the Reload-side call, Games folder
        // remained empty after unlock because the pre-warm Initialize ran
        // while selected_user was invalid (uManager records.bin yielded a
        // fallback but no live entries).
        this->ApplyAppAndSpecialEntries();

        // v2.0.3: defer welcomer ShouldShow/Open to the first frame Main is
        // active (in OnMenuUpdate via first_main_frame_done_ guard).
        // Initialize() runs during MainMenuLayout pre-warm — invoked from
        // ui_MenuApplication.cpp:358 while the Startup login screen is still
        // up — and the prior auto-open here allocated textures + set
        // open_=true while the user hadn't logged in.  See AGENT-WELCOMER-CONFLICT.
#endif
    }

#ifdef QDESKTOP_MODE
    void MainMenuLayout::ApplyAppAndSpecialEntries() {
        ...
        //
        // Fallback path (Cycle A3, SP4.10): when the upstream LoadEntries()
        // returns empty — which happens whenever uMenu boots in a hbloader-
        // hosted / applet-mode context (rc=0x1F800 on ns:am2
        // ListApplicationRecord) — we synthesise the same Entry vector from
        // sdmc:/switch/qos-apps/records.bin pre-written by uManager.nro.
        // See qd_RecordsBin.{hpp,cpp} and the canonical Rust qos_apps.rs in
        // mock-nro-desktop-gui.
        if (!this->qdesktop_icons) {
            return;
        }

        // Perf instrumentation (2026-06-12): time the post-login entry apply
        // so the next boot log shows the REAL login->desktop cost. The boot
        // log's "appinit->paint=9190ms" is dominated by lockscreen dwell time
        // (the user sitting on the user-select screen), NOT this path — which
        // is claimed at ~10-20ms. This confirms it on hardware before any
        // restructure of this (crash-sensitive) entry-load code.
        const u64 t_apply0_ns = armTicksToNs(armGetSystemTick());

        std::vector<ul::menu::Entry> entries =
            ul::menu::LoadEntries(ul::menu::GetActiveMenuPath());
        UL_LOG_INFO("qdesktop: ApplyAppAndSpecialEntries: LoadEntries returned %zu",
                    entries.size());

        // Always load records.bin so we can enrich Application entries
        // whose control.name is empty.  In hbloader/applet mode the NS
        // sysmodule denies nsextGetApplicationControlData (rc=0x1F800),
        // so LoadEntries() returns entries with empty control.name even
        // when .m.json files exist on SD.  records.bin was written by
        // uManager.nro with NACP names from
        // nsextGetApplicationControlData(CacheOnly) which succeeds at
        // full privilege.  Build an app_id → name map from records.bin,
        // then backfill any entry whose control.name is still empty.
        {
            std::vector<ul::menu::Entry> rb_entries;
            const bool rb_ok = ul::menu::qdesktop::LoadEntriesFromRecordsBin(
                    ul::menu::qdesktop::QAPP_RECORDS_BIN_PATH, rb_entries);
            ...
            if (entries.empty() && rb_ok) {
                UL_LOG_INFO("qdesktop: ApplyAppAndSpecialEntries: records.bin"
                ...
                UL_LOG_INFO("qdesktop: ApplyAppAndSpecialEntries: records.bin"
                            " unavailable — display names may fall back"
                            " to hex app_id labels");
            }
        }
        ...
        const u64 t_apply_ms =
            (armTicksToNs(armGetSystemTick()) - t_apply0_ns) / 1'000'000ULL;
        UL_LOG_INFO("qdesktop: ApplyAppAndSpecialEntries took %llu ms (%zu entries) "
                    "[post-login desktop-load cost]",
                    static_cast<unsigned long long>(t_apply_ms), entries.size());
    }
#endif

    void MainMenuLayout::Reload() {
#ifdef QDESKTOP_MODE
        // v3.5.1 stable: re-apply application + special entries on every
        // transition into Main.  The pre-warm Initialize() ran while
        // selected_user was invalid, so the entry vector it built was the
        // records.bin fallback.  After unlock, SetSelectedUser populates
        // g_ApplicationRecords with the live NS data; calling
        // ApplyAppAndSpecialEntries here re-LoadEntries from disk (which
        // now contains the user-specific .m.json files written by
        // InitializeRemainingEntries) and pushes them into the desktop
        // icons element so Games / Apps folders fill out correctly.
        //
        // Cost: one disk scan + records.bin read (~10-20ms on FAT32 SD).
        // No NS IPC — that already ran from SetSelectedUser.
        this->ApplyAppAndSpecialEntries();
