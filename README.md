...
If we missed your name and you should be on this list, open an issue and we will add you. The full attribution chain lives in [CREDITS.md](./CREDITS.md) and the GPLv2 propagation chain is documented in [LICENSE-AUDIT.md](./LICENSE-AUDIT.md).
...
The full version chain, build ritual, and roadmap are in [ROADMAP.md](./ROADMAP.md).
...
GPLv2. The chain of why is documented in [LICENSE-AUDIT.md](./LICENSE-AUDIT.md). Short version: Plutonium and Atmosphere-libs are GPLv2 and they propagate through static linking, so the whole project is GPLv2 too. We did not pick that, the upstream did, but we agree with the choice and we ship under the same terms.
