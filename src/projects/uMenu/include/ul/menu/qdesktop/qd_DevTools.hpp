...
// ── nxlink inbound server (Phase 1) ─────────────────────────────────────────
// Phase 1 of HBmenu absorption: uMenu listens on TCP port 28771 for incoming
// NRO file transfers from the standard `nxlink -s <ip> <file.nro>` host tool.
// Replaces HBmenu's common/netloader.c receiver.

bool IsNxlinkServerActive();
bool TryEnableNxlinkServer();
void DisableNxlinkServer();

// ── Remote shell server (Phase 2) ──────────────────────────────────────────
// Phase 2 of HBmenu absorption: uMenu listens on TCP port 9999 for a
// line-oriented developer shell.  Connect with `telnet <ip> 9999` or
// `nc <ip> 9999` from a host PC to run status/log/launch commands.
