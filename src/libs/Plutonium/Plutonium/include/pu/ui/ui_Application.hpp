...
namespace pu::ui {
    ...
    class Application {
        ...
        protected:
            ...
            std::vector<RenderCallback> post_render_cbs;  // fire after all drawing, before present (screenshot capture)
        ...
        public:
            ...
            /**
             * @brief Adds a callback that fires AFTER all elements/overlays render
             *        but BEFORE the buffer is presented — the only point where the
             *        composed frame can be read back (e.g. a screenshot).
             */
            inline void AddPostRenderCallback(RenderCallback render_cb) {
                this->post_render_cbs.push_back(render_cb);
            }
