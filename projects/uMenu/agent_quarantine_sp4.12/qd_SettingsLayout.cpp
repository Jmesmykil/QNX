...
static void DrawOutlineRect(SDL_Renderer *r, s32 x, s32 y, s32 w, s32 h,
                            u8 rr, u8 gg, u8 bb, u8 aa) {
    SDL_SetRenderDrawBlendMode(r, SDL_BLENDMODE_BLEND);
    SDL_SetRenderDrawColor(r, rr, gg, bb, aa);
    const SDL_Rect rect{ x, y, w, h };
...
void QdSettingsElement::Refresh() {
    ...
    // Access it via extern to avoid re-calling setsys inside the layout.
    {
        // The extern is declared in the file header of qd_DesktopIcons.cpp.
        // We re-declare it here in the same translation unit.
        ...
        // Device nickname.
        strncpy(acc_nickname_, g_GlobalSettings.nickname.nickname,
                sizeof(acc_nickname_) - 1);
        ...
        if (lang_idx >= 0 &&
            ...
            strncpy(acc_language_, lang_names[lang_idx], sizeof(acc_language_));
        } else {
            snprintf(acc_language_, sizeof(acc_language_), "(%d)", lang_idx);
        }

        // Battery lot.
        strncpy(abt_battery_lot_, g_GlobalSettings.battery_lot.lot,
                sizeof(abt_battery_lot_) - 1);
        ...
        if (reg >= 0 && reg < 6) {
            strncpy(abt_region_, region_names[reg], sizeof(abt_region_));
        } else {
...
void QdSettingsElement::BuildRows() {
    ...
    {
        extern ul::menu::ui::GlobalSettings g_GlobalSettings;
        strncpy(account_rows_[2].value,
                g_GlobalSettings.nickname.nickname,
                sizeof(account_rows_[2].value) - 1);
    }
