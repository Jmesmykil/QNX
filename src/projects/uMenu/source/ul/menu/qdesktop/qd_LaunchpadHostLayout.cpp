...
namespace ul::menu::qdesktop {
    ...
    void QdLaunchpadHostLayout::LoadSfx() {
        // QOS-AUDIO-BOOTCHIME (2026-06-19): Preload the boot chime into the engine
        // SFX cache HERE (early, before the MenuApplication render callback fires).
        // The synchronous Mix_LoadWAV inside the render callback was stalling the
        // first frame after login (~tens of ms on SD card).  PreloadSfx() populates
        // the path-keyed cache; the subsequent LoadSfx() call in the render callback
        // becomes a cache-hit (pointer return, zero I/O).
        pu::audio::PreloadSfx(ul::menu::ui::TryGetActiveThemeResource("sound/Launchpad/BootChime.wav"));
