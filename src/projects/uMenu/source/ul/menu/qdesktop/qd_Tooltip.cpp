...
#include <ul/menu/qdesktop/qd_LayoutConstants.hpp>   // TOOLTIP_* constants (D13 fix)

namespace ul::menu::qdesktop {

// ── Layout constants ──────────────────────────────────────────────────────────
// D13 fix: kScreenW=1920 and kScreenH=1080 were hardcoded here instead of
// using SCREEN_W/H.  Now aliased from qd_LayoutConstants.hpp so any resolution
// change only needs one edit.
// Padding inside the background rect (px).
static constexpr int kPadX = TOOLTIP_PAD_X;       // 10 left + right padding each
static constexpr int kPadY = TOOLTIP_PAD_Y;       //  6 top + bottom padding each

// Minimum gap above the anchor when prefer_above=true.
// The tooltip bottom lands at (anchor_y - kGapAbove).
static constexpr int kGapAbove = TOOLTIP_GAP_ABOVE;  // 6

// Minimum gap below the anchor when rendering below (fallback).
static constexpr int kGapBelow = TOOLTIP_GAP_BELOW;  // 6
...
// Screen bounds — from qd_LayoutConstants.hpp (= SCREEN_W / SCREEN_H).
static constexpr int kScreenW = TOOLTIP_SCREEN_W;  // 1920
static constexpr int kScreenH = TOOLTIP_SCREEN_H;  // 1080
