...
#include <atomic>    // W17-FIX1: deferred InitializeEntries thread flag
#include <thread>    // W17-FIX1: background enumeration thread
#include <vector>    // W17-FIX1: deferred_entries_ storage
#ifdef QDESKTOP_MODE
...
#include <ul/menu/qdesktop/qd_WindowOverlay.hpp>
...
namespace ul::menu::ui {
    ...
    class MainMenuLayout : public IMenuLayout {
        ...
        private:
            ...
            qdesktop::QdWindowOverlay::Ref qdesktop_window_overlay_;  // W10-BUG1B
            qdesktop::QdHotCornerOverlay::Ref qdesktop_overlay_;
            qdesktop::QdHotCornerRightOverlay::Ref qdesktop_right_overlay_;
            ...
            // v2.0.3: gate ShouldShow/Open to the first frame Main is ACTIVE,
            // not the Initialize() call (which runs during Startup pre-warm
            // and was opening the modal invisibly while the login screen was
            // still up — leaking textures and confusing the lifecycle).
            bool first_main_frame_done_ = false;

            // W17-FIX1: deferred InitializeEntries storage.
            // Background thread enumerates ns:am and stores results here.
            // Atomic flag is set to true when the vector is fully written.
            // OnMenuUpdate reads the flag on the main thread and calls
            // SetApplicationEntries / SetSpecialEntries from there (thread-safe).
            std::vector<ul::menu::Entry> deferred_entries_;
            std::atomic<bool> deferred_entries_ready_ { false };
            bool deferred_entries_applied_ = false;
#endif
        ...
        public:
            ...
            // OnHomeButtonLongPress NOT overridden: HW test 2026-05-19
            // proved long-press HOME is HOS-reserved (Nintendo's
            // power/airplane menu always wins).  Dev mini-menu is reached
            // via the double-tap HOME path inside OnHomeButtonPress
            // instead.  IMenuLayout's default OnHomeButtonLongPress
            // (no-op true) is fine.
            ...
            // v3.5.1 stable: shared helper invoked from BOTH Initialize() (boot
            // pre-warm) and Reload() (every transition into Main after unlock /
            // user-change).  Loads .m.json entries from the active user's menu
            // dir, falls back to records.bin if NS IPC denied us names, then
            // calls SetApplicationEntries + SetSpecialEntries on the desktop
            // icons element.  Idempotent: safe to call multiple times per
            // session, the icons element truncates+rebuilds the app-slot range.
            void ApplyAppAndSpecialEntries();
