...
// B1.1: Extract the NACP application name from an NRO's ASET section.
//
// Algorithm:
//   1. Open the NRO via kernel IPC (fsFsOpenFile).
//   2. Read NRO header; verify NRO0 magic; read nro_size.
//   3. Read ASET header at absolute offset nro_size; verify ASET magic.
//   4. Read the NACP descriptor at ASET+0x18: nacp_off (u64), nacp_size (u64).
//   5. Bounds-check: nacp_size must equal sizeof(NacpStruct) and
//      (nacp_off + nacp_size) must not exceed (file_size - nro_size).
//   6. Read NacpStruct at absolute offset (nro_size + nacp_off).
//   7. Pick the display name with the same language-selection logic as
//      PickNacpName in qd_CheatTitleResolver.cpp:
//        a. current system language (setGetSystemLanguage)
//        b. American English (SetLanguage_ENUS)
//        c. any non-empty lang entry
//   8. Truncate result to 63 chars + NUL if longer.
//
// Returns: non-empty std::string with the application name on success.
//          Empty string on any failure (no ASET, no NACP descriptor, empty
//          name, malformed NRO, I/O error) — caller keeps the filename stem.
