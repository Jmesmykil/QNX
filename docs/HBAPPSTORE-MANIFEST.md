...
>
> **Version targeted:** Q OS uMenu v3.0.2 (Makefile `VERSION_MAJOR=1 VERSION_MINOR=2 VERSION_MICRO=3`).
...
```json
{
  ...
  "changelog": "v3.0.2: Q OS desktop layout (QDESKTOP_MODE), auto-grid icon placement, 6-slot dock, layered top bar, login screen, Q OS-branded special icons. Upstream uLaunch v3.0.2 base.",
...
| `version` | `3.0.2` | From `src/Makefile` `VERSION_MAJOR=1 VERSION_MINOR=2 VERSION_MICRO=3`. Update this for every release. |
...
`uLaunch-v3.0.2.zip` containing three top-level directories from `SdOut/`:

```
qos-umenu-v3.0.2.zip
├── atmosphere/
│   └── contents/
│       └── 0100000000001000/
...
- Release: https://github.com/Jmesmykil/QOS/releases/tag/qos-umenu-v3.0.2

Attaching: icon.png (256×256), screen.png (1280×720), qos-umenu-v3.0.2.zip
```
