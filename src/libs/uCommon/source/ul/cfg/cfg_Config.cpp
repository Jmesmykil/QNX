...
#include <switch.h>  // v2.6.0 — fsdevCommitDevice for SD flush after SaveConfig
...
namespace ul::cfg {
    ...
    void CacheActiveTheme(const Config &cfg) {
        ...
        // v2.8.0 — mirror the v2.6.0 SaveConfig fix.  fs::WriteFile in the
        // loop above does fopen+fwrite+fclose, but libnx's FAT layer buffers
        // writes internally and only commits on fsdevCommitDevice or process
        // exit.  Without this commit, uMenu B's CacheActiveTheme at boot
        // (called via reload_theme_cache in main.cpp:291) reads the cache
        // we wrote SECONDS AGO from the dying uMenu A process — but if the
        // FAT buffer hasn't flushed, what's on the SD is stale or partial.
        // The same class of bug that lost SaveFolderThemePack writes pre-v2.5.1
        // and SaveConfig writes pre-v2.6.0.  Cheap to call (one IPC), keeps
        // the cache atomic with respect to the next boot.
    ...
    void SaveConfig(const Config &cfg) {
        ...
        // v2.6.0 — CRITICAL: flush the SD-card backing store. Without this,
        // libnx keeps the config write in its in-memory FAT cache and the
        // write only takes effect in-process. When the menu process
        // terminates for RestartMenu, the buffered write is LOST and the
        // next boot loads the OLD config — same root-cause bug pattern as
        // SaveFolderThemePack (fixed in v2.5.1). For theme picks this meant
        // SetActiveTheme({}) (clearing the .ultheme) didn't actually persist:
        // every restart re-loaded the prior .ultheme silently.
        const Result commit_rc = fsdevCommitDevice("sdmc");
        if (R_FAILED(commit_rc)) {
            UL_LOG_WARN("cfg::SaveConfig: fsdevCommitDevice rc=0x%x", commit_rc);
        }
    }
