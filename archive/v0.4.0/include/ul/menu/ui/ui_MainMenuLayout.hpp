...
namespace ul::menu::ui {
    ...
    // QOS-PATCH-008: Finder-style focus surface enumeration.
    // RouteDpadInput uses this to dispatch D-pad events to the correct surface.
    enum class FocusSurface : u8 {
        DesktopGrid = 0,  // Default — icon grid receives D-pad input
        Dock,             // Horizontal dock strip at bottom
        Window,           // Modal/overlay window (unused in v0.3.0, reserved)
        OnScreenKeyboard, // OSK layer (Fix 6, deferred to v0.4.0)
        Overlay,          // Any fullscreen overlay
    };
    ...
    class MainMenuLayout : public IMenuLayout {
        ...
        private:
            ...
            // QOS-PATCH-009: Cold Plasma Cascade procedural wallpaper (layer 0)
            PlasmaWallpaperElement::Ref plasma_wallpaper;

            // QOS-PATCH-005/008: Dock and focus-surface state
            DockElement::Ref dock_element;
            FocusSurface current_surface;
            FocusSurface prev_surface;
            s32 selected_dock_index;
            pu::ui::elm::Rectangle::Ref focus_ring;
            ...
            // QOS-PATCH-008: Route D-pad input to the currently focused surface.
            // Returns true if the event was consumed.
            bool RouteDpadInput(const u64 keys_down);

            // QOS-PATCH-008: True when the cursor is on the top row of the grid (row 0)
            inline bool AtTopRow() const {
                return false; // grid rows grow downward; top row is row 0 in 1-row configs
                // For multi-row: (cur_entry_idx % g_EntryHeightCount == 0) — evaluated in .cpp
            }

            // QOS-PATCH-005: Helper to update dock hover from touch or focus index
            void UpdateDockHoverFromFocus();
        ...
        public:
            ...
            // QOS-PATCH-007: Dedicated chainload entry point.
            // Dispatches to smi::LaunchApplication or smi::LaunchHomebrewApplication
            // depending on the entry type.  Emits EVENT UX_LAUNCH.
            void HandleEntrySelection(const Entry &selected);

            // QOS-PATCH-008: Expose current focus surface for external query
            inline FocusSurface GetCurrentSurface() const {
