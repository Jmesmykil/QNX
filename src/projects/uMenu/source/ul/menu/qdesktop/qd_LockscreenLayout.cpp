      ...
      hint_bar_tex_(nullptr)
{
    ...
    // v3.7.2: Medium font + bright accent (was Small + muted text_secondary,
    // which was invisible on the Dark theme's near-black bg → looked like a
    // black screen with no cue to press A).
                                   ...
                                   const s32 /*x*/, const s32 /*y*/)
{
    ...
    // v3.7.2: confirm the lockscreen actually paints during boot (rules out a
    // pre-first-render gate as the black-screen cause vs. the scrim/contrast).
    {
        static bool ls_first_paint_logged = false;
        if (!ls_first_paint_logged) {
            UL_LOG_INFO("lockscreen: QdLockscreenElement OnRender FIRST paint (scrim a=96, accent prompt)");
            ls_first_paint_logged = true;
        }
    }
    ...
    // v3.7.2: alpha 192→96.  At 192 over the Dark theme's near-black wallpaper
    // (desktop_bg #050010) the whole screen rendered effectively black; 96 dims
    // the wallpaper for text contrast while keeping the art-directed wallpaper
    // clearly visible (never a black screen).
    SDL_SetRenderDrawBlendMode(r, SDL_BLENDMODE_BLEND);
                                     ...
                                     const pu::ui::TouchPoint touch_pos)
{
    ...
    if (should_unlock) {
        UL_LOG_INFO("lockscreen: unlock (keys_down=0x%llx touch=%s) — loading Main (fast)",
                    (unsigned long long)keys_down,
                    touch_pos.IsEmpty() ? "no" : "yes");
        QdAudio::Play(DesktopSfxEvent::LockscreenUnlockOk);
        if (g_MenuApplication) {
            // W17-FIX3: skip fade for lockscreen → Main transition.
            // The lockscreen already shows the wallpaper; the Main desktop also
            // shows the same wallpaper.  A cross-fade between two identical
            // backgrounds is perceptible lag without visual benefit (~700ms for
            // the default 20-step FadeOut + FadeIn pair at 60fps).
            // Passing fade=false makes the transition instant — the user sees
            // the desktop appear immediately on button press.
            g_MenuApplication->LoadMenu(ul::menu::ui::MenuType::Main, /*fade=*/false);
        }
    }
}

bool QdLockscreenLayout::OnHomeButtonPress() {
    // W17-FIX3: same no-fade rationale as OnMenuInput unlock path.
    UL_LOG_INFO("lockscreen: OnHomeButtonPress -> returning to MainMenu (fast)");
    g_MenuApplication->LoadMenu(ul::menu::ui::MenuType::Main, /*fade=*/false);
    return true;
}
