...
#include <ul/system/mem/mem_Telemetry.hpp> // QOS-PATCH-010: memory telemetry
...
namespace {
    ...
    Result LaunchMenu(const ul::smi::MenuStartMode st_mode) {
        ...
        return ecs::RegisterLaunchAsApplet(la::GetMenuProgramId(), static_cast<u32>(st_mode), "/qos-shell/bin/uMenu", std::addressof(g_CurrentStatus), sizeof(g_CurrentStatus));
    }
        ...
        switch(action.type) {
            ...
            case ActionType::LaunchHomebrewLibraryApplet: {
                if(!la::IsActive()) {
                    ...
                    UL_RC_ASSERT(ecs::RegisterLaunchAsApplet(hb_applet_takeover_program_id, 0, "/qos-shell/bin/uLoader/applet", &action.launch_loader.target_input, sizeof(action.launch_loader.target_input)));
            ...
            case ActionType::LaunchHomebrewApplication: {
                if(!la::IsActive()) {
                    ...
                    UL_RC_ASSERT(ecs::RegisterLaunchAsApplication(action.launch_homebrew_application.app_id, "/qos-shell/bin/uLoader/application", &action.launch_homebrew_application.app_target_input, sizeof(action.launch_homebrew_application.app_target_input), g_SelectedUser));
    ...
    void Initialize() {
        ...
        ul::system::mem::LogMemoryStats("boot"); // QOS-PATCH-010
        g_CurrentRecords = ul::os::ListApplicationRecords();
        ul::system::app::InitializeControlCache(g_CurrentRecords);
        ul::system::mem::LogMemoryStats("post-cache-init"); // QOS-PATCH-010
...
namespace ams {
    ...
    namespace init {
        ...
        void InitializeSystemModule() {
            ...
            // QOS-PATCH-003: Q OS fork identity banner emitted at boot for telemetry + hw verification.
            // Copyright (c) XorTroll (upstream). Patch: Q OS fork, GPLv2.
            UL_LOG_INFO("Q OS uMenu v0.2.1 (fork of uLaunch by XorTroll, GPLv2, personal-use)");
        }
    ...
    void Main() {
        ...
        ul::system::mem::LogMemoryStats("pre-menu-spawn"); // QOS-PATCH-010
