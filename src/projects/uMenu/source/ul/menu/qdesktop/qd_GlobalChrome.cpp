...
#include <ul/menu/qdesktop/qd_Theme.hpp>           // v2.6.0 — top bar + dock chrome read g_QdTheme.topbar_bg / dock_bg
...
void QdGlobalChrome::RenderTopBar(SDL_Renderer *r) {
    ...
    // -- Pass 1: translucent top-bar background (theme-aware, v2.6.0)
    SDL_SetRenderDrawBlendMode(r, SDL_BLENDMODE_BLEND);
    {
        const auto &tb = g_QdTheme.topbar_bg;
        SDL_SetRenderDrawColor(r, tb.r, tb.g, tb.b, 0xC0u);  // theme tint, slightly translucent
        SDL_Rect topbar_bg { 0, 0, 1920, kChromeTopBarH };
        SDL_RenderFillRect(r, &topbar_bg);
    }

    // -- Pass 2: 1-px bottom border hairline (accent-colored, low alpha)
    {
        const auto &ac = g_QdTheme.accent;
        SDL_SetRenderDrawColor(r, ac.r, ac.g, ac.b, 0x60u);
...
void QdGlobalChrome::RenderDock(SDL_Renderer *r) {
    ...
    // Translucent backdrop (theme-aware, v2.6.0)
    SDL_SetRenderDrawBlendMode(r, SDL_BLENDMODE_BLEND);
    {
        const auto &db = g_QdTheme.dock_bg;
        SDL_SetRenderDrawColor(r, db.r, db.g, db.b, 0xA0u);  // theme tint, semi-translucent
        SDL_Rect dock_panel { 0, kChromeDockNominalTop, 1920, kChromeDockH };
        SDL_RenderFillRect(r, &dock_panel);
    }

    // 1-px top border hairline (accent-colored)
    {
        const auto &ac = g_QdTheme.accent;
        SDL_SetRenderDrawColor(r, ac.r, ac.g, ac.b, 0x80u);
