// qd_MonitorHostLayout.hpp — IMenuLayout host wrapper for QdMonitorLayout (Monitor Element).
//
// Why this class exists
// ---------------------
// QdMonitorLayout is a pu::ui::elm::Element despite the "Layout" suffix in its
// class name.  Plutonium's pu::ui::Layout::Add() is the canonical way to attach
// an Element to a screen, so QdMonitorLayout must be wrapped in a Layout subclass.
//
...
// Crashed exactly this way for the Vault surface before K-vaultfix.
// See qd_VaultHostLayout.hpp for the full crash chain description.
//
// QdMonitorHostLayout is a real IMenuLayout that owns the QdMonitorLayout
// element and satisfies the IMenuLayout obligations the dispatcher relies on.
...
namespace ul::menu::qdesktop {
    ...
    class QdMonitorHostLayout : public ul::menu::ui::IMenuLayout {
        public:
            // Construct with an already-built QdMonitorLayout element.  The host
            // takes ownership via shared_ptr and adds the element as a child.
            explicit QdMonitorHostLayout(QdMonitorLayout::Ref monitor_element);
            ...
            PU_SMART_CTOR(QdMonitorHostLayout)
            ...
            // Monitor has no dedicated sfx surface today — these are intentional
