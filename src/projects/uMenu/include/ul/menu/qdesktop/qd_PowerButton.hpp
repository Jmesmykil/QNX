...
public:
    ...
    enum class Kind : u8 { Restart, Shutdown, Sleep, Hekate, HekateUms, Custom };
...
private:
    ...
    // Touch click-vs-drag state — same pattern as QdUserCardElement.
    // v2.0.3: was_touch_active_ edge-tracker eliminates the per-frame jitter
    // sensitivity that was making login-screen taps miss.  Without it, ANY
    // frame where finger position fluctuates outside the rect (capacitive
    // jitter on Erista) permanently cleared press_inside_ for that gesture.
    // last_touch_x_/y_ track most-recent position for the TouchUp distance
    // check (cumulative drag, not per-frame).
