...
namespace ul::menu::ui {
    ...
    namespace {
        ...
        // ── A4-OPT-05: TryGetActiveThemeResource path memoization ────────────
        //
        // Repeated callers (e.g. TryFindLoadImage iterating ImageFormatList for
        // each of ~6 folder-icon entries = ~48 stat()s on first frame) resolve
        // the same resource_base repeatedly.  Cache the resolved path keyed on
        // resource_base so subsequent calls within the same theme session are
        // O(1) hash lookups rather than stat() calls.
        //
        // Invalidation: ClearActiveThemeResourceCache() must be called whenever
        // the active theme changes (see SetActiveTheme in ui_Common.hpp).
        // The cache is process-local and never persisted.
        static std::unordered_map<std::string, std::string> g_theme_resource_cache;
    ...
    // Clear the TryGetActiveThemeResource memo cache.
    // Must be called on every theme change so stale paths are not served.
    void ClearActiveThemeResourceCache() {
        g_theme_resource_cache.clear();
    }

    std::string TryGetActiveThemeResource(const std::string &resource_base) {
        // ── A4-OPT-05: memo cache lookup ─────────────────────────────────────
        auto it = g_theme_resource_cache.find(resource_base);
        if (it != g_theme_resource_cache.end()) {
            return it->second;
        }
        // ─────────────────────────────────────────────────────────────────────
        ...
        g_theme_resource_cache.emplace(resource_base, path);
        return path;
    }
    ...
    void ShowPowerDialog() {
        ...
        if(sopt == 0) {
            ul::menu::qdesktop::QdAudio::Play(ul::menu::qdesktop::DesktopSfxEvent::PowerSleepConfirm);
            msg = ul::system::GeneralChannelMessage::Unk_Sleep;
        }
        else if(sopt == 1) {
            ul::menu::qdesktop::QdAudio::Play(ul::menu::qdesktop::DesktopSfxEvent::PowerShutdownConfirm);
