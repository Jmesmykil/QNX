...
public:
    ...
    //   1. Uses the NS session opened by main.cpp:222 (W5-TRANSITIONS #4:
    //      duplicate nsInitialize removed; ns_open_ is always true at init).
...
private:
    ...
    // A4-OPT-06: LRU eviction — cap the cache at kMaxEntries to bound VRAM.
    // lru_order_ holds program_ids from MRU (front) to LRU (back).
    // Get() moves a hit to the front; on insert, evict the back when full.
    // ⚠ SDL_DestroyTexture is only safe on the render thread; Get() is called
    //   from the render path so eviction here is safe (see A4-RF-03).
    static constexpr size_t kMaxEntries = 32;

    bool ns_open_;
    std::unordered_map<u64, SDL_Texture *> cache_;
    std::list<u64>                          lru_order_;  // MRU front, LRU back
    // W6-LEDGER: per-entry ledger handles (0 = not tracked or nullptr result).
    std::unordered_map<u64, uint64_t> ledger_handles_;
};
