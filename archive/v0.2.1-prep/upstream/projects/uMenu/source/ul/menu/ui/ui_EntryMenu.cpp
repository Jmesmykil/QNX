...
namespace ul::menu::ui {
    ...
    bool EntryMenu::LoadEntry(const u32 idx) {
        if(idx < this->cur_entries.size()) {
            ...
            if(!entry.Is<EntryType::Invalid>()) {
                ...
                // Layer 3: clear dirty flag now that the texture slot has been (re-)populated.
                if(idx < this->entry_icon_dirty.size()) {
                    this->entry_icon_dirty.at(idx) = false;
                }
    ...
    void EntryMenu::OnRender(pu::ui::render::Renderer::Ref &drawer, const s32 x, const s32 y) {
        ...
        */
        ...
        // Layer 3: allow at most one dirty-tile texture upload per frame to avoid bursting
        // the SDL/GL texture pool after a FlushIconCache() call on HOME-return.
        bool dirty_upload_done_this_frame = false;
        for(u32 i = 0; i < entry_render_count; i++) {
            ...
            if(entry_i < this->entry_icons.size()) {
                // Layer 3: if this tile is dirty, only allow its upload if no other tile
                // has been uploaded this frame; otherwise render the empty placeholder.
                const bool tile_dirty = (entry_i < this->entry_icon_dirty.size()) && this->entry_icon_dirty.at(entry_i);
                if(tile_dirty) {
                    if(!dirty_upload_done_this_frame) {
                        // Permit the upload — LoadEntry will clear the flag.
                        this->LoadEntry(entry_i);
                        dirty_upload_done_this_frame = true;
                    }
                    // Regardless, read the (possibly just-loaded) handle.
                }
    ...
    void EntryMenu::MoveTo(const std::string &new_path, const bool force_pop_idx) {
        ...
        for(u32 i = 0; i < this->entry_total_count; i++) {
            ...
            this->entry_icon_dirty.push_back(false);
    ...
    void EntryMenu::FlushIconCache() {
        // Layer 2: clear all cached icon textures so the next render cycle performs
        // lazy re-upload one tile at a time instead of bursting all uploads at once.
        // This prevents nouveau texture pool exhaustion on resume from a GL/GLES2 applet.
        for(auto &icon: this->entry_icons) {
            icon = {};
        }
        // Layer 3: mark every tile dirty so OnRender uploads at most one texture per
        // frame instead of re-uploading the full visible set in a single burst.
        // std::vector<bool> yields a proxy reference; use index assignment to avoid
        // the _Bit_reference rvalue bind error with auto&.
        for(u32 i = 0; i < this->entry_icon_dirty.size(); i++) {
            this->entry_icon_dirty[i] = true;
        }
        this->pending_load_img_done = false;
        this->pending_load_img_entry_start_idx = 0;
        this->pending_load_img_entry_ext_idx = 0;
    }
