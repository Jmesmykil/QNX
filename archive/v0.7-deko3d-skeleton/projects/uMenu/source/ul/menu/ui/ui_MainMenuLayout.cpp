...
namespace ul::menu::ui {
    ...
    bool MainMenuLayout::OnHomeButtonPress() {
        ...
        if(g_GlobalSettings.IsSuspended()) {
            ...
            // Layer 4: drain any pending SDL/GL texture-pool operations by issuing an
            // explicit RenderClear before re-uploading icons.  This gives the GPU a chance
            // to retire outstanding draw calls from the previous applet session so that the
            // subsequent FlushIconCache() + lazy re-upload runs against a clean pool.
            SDL_RenderClear(pu::ui::render::GetMainRenderer());

            // Layer 2: flush icon cache before rewind so re-entry from a GL/GLES2 applet
            // re-uploads icons lazily one-at-a-time rather than in a burst that exhausts
            // the nouveau texture pool (root cause of Data Abort 2168-0002 / 0x4a8).
            // Layer 3 dirty flags are set inside FlushIconCache().
            this->entry_menu->FlushIconCache();
    ...
    void MainMenuLayout::HandleEntrySelection(const Entry &selected) {
        ...
        UL_LOG_INFO("HandleEntrySelection entry_type=%u", static_cast<u32>(selected.type));
