            ...
            switch(id) {
                case ConfigEntryId::MenuTakeoverProgramId: {
                    if constexpr(std::is_same_v<T, u64>) {
                        ...
                        // (2026-06-20: tested eShop here as a layer-2 experiment — am rejects the
                        //  2nd in-session relaunch with the SAME error 2128-0035 regardless of applet,
                        //  so the AppletId is NOT the cause. Reverted to PhotoViewer.)
