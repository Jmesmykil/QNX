...
namespace ul::menu::ui {
    ...
    // A4-OPT-05: clear the TryGetActiveThemeResource memo cache.
    // Call on every theme change so stale paths are not served.
    void ClearActiveThemeResourceCache();
    ...
    struct MenuBgmEntry {
        ...
        // v2.0.3-A2: 1500 → 250 ms to match the HOME applet's perceived boot-
        // chime fade per qos-native-logic-first.md §5.  Five menu BGM entries
        // (ui_Common.hpp:249-253) inherit this fallback in TryParseBgmEntry
        // (ui_Common.cpp:298,305) when no explicit bgm_fade_in_ms key is set.
        // Risk: audio-pop on Tegra X1 (P-A class, items 5/40/41/118) — T2 chime-
        // once HW test mandatory before ship.
        static constexpr u32 DefaultBgmFadeInMs = 250;
    ...
    struct GlobalSettings {
        ...
        inline void SetActiveTheme(const ul::cfg::Theme &theme) {
            ...
            this->SaveConfig();
            // v2.6.0 — when clearing the active .ultheme (theme is {}), also
            // wipe sdmc:/ulaunch/cache/active/ so TryGetActiveThemeResource()
            // stops resolving to stale .ultheme icons/sounds/fonts after a
            // theme pick that should have removed them. Previously the cache
            // dir lingered, so the user kept seeing the old .ultheme's icons
            // even after the config said "no active theme."
            if (this->active_theme.name.empty()) {
                ul::cfg::RemoveActiveThemeCache();
            }
            // A4-OPT-05: invalidate the TryGetActiveThemeResource memo cache
            // so the new theme's paths are resolved fresh on next access.
            ClearActiveThemeResourceCache();
        }
