...
// ── Fix 2: SD-root NRO path construction ─────────────────────────────────────
// These tests verify that the path-building logic in ScanNros() for the
// sdmc:/ scan (v1.6.11 Fix 2) produces correct non-empty paths.
// The actual opendir() and NRO scanning are device-only; this tests the
// snprintf format string behaviour in isolation.

static void test_sdroot_nro_path_hbmenu() {
    // snprintf(nro_path, sizeof(nro_path), "sdmc:/%s", fname)
    // For fname = "hbmenu.nro" the result must start with "sdmc:/" and end with ".nro".
...
static void test_sdroot_nro_path_distinct_from_switch_dir() {
    // Verify that "sdmc:/<fname>" != "sdmc:/switch/<fname>" so that SD-root and
    // switch-subdir entries do not collide in the icon cache.
    const char *fname = "hbmenu.nro";
    char root_path[768];
...
static void test_sdroot_nro_stem_extraction() {
    // Stem extraction: fname without ".nro" suffix, same logic as ScanNros().
    const char *fname = "hbmenu.nro";
...
int main() {
    ...
    // Fix 2: SD-root NRO path construction (v1.6.11).
