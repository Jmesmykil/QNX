...
#include <ul/menu/qdesktop/qd_LayoutConstants.hpp>  // DROPDOWN_LEFT_*, DROPDOWN_ROW_H etc (D9 fix)
#include <ul/menu/qdesktop/qd_DesktopIcons.hpp>   // v1.10.1: window-opener back-ref
#include <ul/menu/qdesktop/qd_Launchpad.hpp>     // LP_HOTCORNER_W, LP_HOTCORNER_H (backward compat)
...
// v2.6.0 — dropdown panel/hover/border resolve from g_QdTheme at call site.
// Macros expand to SDL_Color literals so kFoo.r/.g/.b/.a stays valid at every
// existing call site. Picking a theme now visibly changes both top hot-corner
// dropdowns instead of leaving them always-navy-with-cyan-border.
...
// ── Layout constants ─────────────────────────────────────────────────────────
// D9 fix: local constants now alias the SSOT tokens in qd_LayoutConstants.hpp.
// The right dropdown references the SAME shared tokens, so any future size
...
static constexpr int kPadH     = DROPDOWN_PAD_H;    // 16 left text indent
static constexpr int kPanelW   = DROPDOWN_LEFT_W;   // 280 panel width
static constexpr int kRadius   = DROPDOWN_RADIUS;   //  8 v1.9.4: rounded-corner radius (= kPadV
...
void QdHotCornerDropdown::Open(SDL_Renderer *r) {
    ...
    // widget at y=HC_VISUAL_H (72).  This fixes two v1.9.1 HW-test issues:
    ...
    // D9 fix: DROPDOWN_LEFT_X=0, DROPDOWN_LEFT_Y=HC_VISUAL_H=72 (was bare LP_HOTCORNER_H).
    panel_x_ = DROPDOWN_LEFT_X;
    panel_y_ = DROPDOWN_LEFT_Y;
    panel_w_ = kPanelW;
    ...
    prev_cursor_x_ = -1;
    prev_cursor_y_ = -1;
    QdAudio::Play(DesktopSfxEvent::HotCornerActivate);
    UL_LOG_INFO("qdesktop: hot-corner dropdown opened");
}

// BUG-7 — see qd_HotCornerDropdown.hpp doc.
void QdHotCornerDropdown::SetSkipFirstLift() {
...
// v1.10.3.2: Fire the currently-hovered row without requiring the cursor to be
// inside the panel x-bounds.  Called by the ZR handler in qd_DesktopIcons when
// TryClickAt returns false because the cursor drifted outside [0, panel_w_) but
// UpdateHover still set hovered_ on a prior no-touch frame.
bool QdHotCornerDropdown::FireHovered() {
    if (!open_) return false;
    if (hovered_ < 0 || hovered_ >= kDropdownItems) return false;
    if (disabled_[hovered_]) return false;
    const int fired = hovered_;
    hovered_ = -1;  // clear so stale hover doesn't double-fire
    FireAction(fired);
    return true;
}
...
void QdHotCornerDropdown::FireAction(int i) {
    ...
    switch (i) {
        case 0:  // Open Launchpad — full-screen takeover (no window form)
            g_MenuApplication->LoadMenu(ul::menu::ui::MenuType::Launchpad);
            break;
        case 1:  // Settings — opens as a floating window in v1.10
            // v2.3.5: ONLY route through QdSettings.  The legacy
            // LoadMenu(MenuType::Settings) fallback crashed in fsdev
            // unwind (mesosphere reported "svc 0x6f").  If the desktop
            // ref is missing, no-op rather than crash.
            if (desktop_icons_ref_ != nullptr) {
                desktop_icons_ref_->OpenSettingsWindow();
            } else {
                UL_LOG_WARN("hotcorner: Settings clicked but desktop_icons_ref_ is null; skipping");
            }
            break;
        case 2:  // Power — modal dialog (already a popup)
            ::ul::menu::ui::ShowPowerDialog();
            break;
        case 3:  // Notifications — disabled; should not be reachable
            break;
        case 4:  // About — opens as a floating window in v1.10
            if (desktop_icons_ref_ != nullptr) {
                desktop_icons_ref_->OpenAboutWindow();
            } else {
...
bool QdHotCornerDropdown::HandleInput(u64 keys_down, u64 keys_held,
    ...
    // See qd_HotCornerDropdown.hpp doc on skip_first_lift_.
    if (just_released && skip_first_lift_) {
        skip_first_lift_ = false;
        hovered_ = -1;
        UL_LOG_INFO("qdesktop: hot-corner dropdown first-lift skipped");
        return true;
    }
