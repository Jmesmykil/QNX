...
void QdAboutElement::Refresh() {
    ...
    // ── Row 0: Q OS uMenu version ─────────────────────────────────────────
    snprintf(rows_[0].label, sizeof(rows_[0].label), "Q OS uMenu");
    ...
    // ── Row 14: SD Card free / total space (Sphaira-absorb sprint 1) ─────
    // Opens the SD card fs handle via fsOpenSdCardFileSystem, queries
    // GetFreeSpace and GetTotalSpace on the root path "/", then closes.
    snprintf(rows_[14].label, sizeof(rows_[14].label), "SD Card");
    {
        FsFileSystem sd_fs;
        const Result rc_open = fsOpenSdCardFileSystem(&sd_fs);
        if (R_SUCCEEDED(rc_open)) {
            s64 free_bytes  = 0;
            s64 total_bytes = 0;
            const Result rc_free  = fsFsGetFreeSpace(&sd_fs,  "/", &free_bytes);
            const Result rc_total = fsFsGetTotalSpace(&sd_fs, "/", &total_bytes);
            fsFsClose(&sd_fs);
            if (R_SUCCEEDED(rc_free) && R_SUCCEEDED(rc_total) && total_bytes > 0) {
                // Convert to GiB for readability; keep 1 decimal place.
                ...
                snprintf(rows_[14].value, sizeof(rows_[14].value),
                         "%.1f / %.1f GiB free", free_gb, total_gb);
            } else {
                snprintf(rows_[14].value, sizeof(rows_[14].value), "n/a");
            }
        } else {
            snprintf(rows_[14].value, sizeof(rows_[14].value), "n/a");
        }
    ...
    // ── Row 15: Wi-Fi SSID / connection state (Sphaira-absorb sprint 1) ──
    // Opens nifm:u (user session) to query the current IP and connection status.
    // We report SSID-equivalent via IP + status since nifmGetCurrentNetworkProfile
    // requires higher privilege on retail; fallback to IP display is graceful.
    snprintf(rows_[15].label, sizeof(rows_[15].label), "Wi-Fi");
    {
        if (R_SUCCEEDED(nifmInitialize(NifmServiceType_User))) {
            ...
            const Result rc_st = nifmGetInternetConnectionStatus(
                &conn_type, &signal_str, &conn_status);
            if (R_SUCCEEDED(rc_st) &&
                conn_status == NifmInternetConnectionStatus_Connected &&
                conn_type   == NifmInternetConnectionType_WiFi) {
                u32 ip_u32 = 0;
                if (R_SUCCEEDED(nifmGetCurrentIpAddress(&ip_u32)) && ip_u32 != 0) {
                    snprintf(rows_[15].value, sizeof(rows_[15].value),
                             "Connected  (%u.%u.%u.%u)",
                             (ip_u32 >>  0) & 0xFF,
                             (ip_u32 >>  8) & 0xFF,
                             (ip_u32 >> 16) & 0xFF,
                    ...
                    snprintf(rows_[15].value, sizeof(rows_[15].value), "Connected");
                }
            } else if (R_SUCCEEDED(rc_st) &&
                       conn_type == NifmInternetConnectionType_Ethernet) {
                snprintf(rows_[15].value, sizeof(rows_[15].value), "Ethernet (not Wi-Fi)");
            } else {
                snprintf(rows_[15].value, sizeof(rows_[15].value), "Not connected");
            }
            nifmExit();
        } else {
            snprintf(rows_[15].value, sizeof(rows_[15].value), "n/a");
        }
    }
...
QdAboutElement::AboutGeo QdAboutElement::ComputeGeo(s32 ox, s32 oy) const {
    AboutGeo g;
    ...
    // content area.  All positions are expressed in screen coordinates so
    // they remain correct when the window is dragged to any position.
    const s32 max_card_w = 1400;
    const s32 max_card_h = 800;
    const s32 avail_w = GetNaturalW() - 96;   // 48 px margin each side
    const s32 avail_h = GetNaturalH() - 96;
    g.card_w = (avail_w < max_card_w) ? avail_w : max_card_w;
    g.card_h = (avail_h < max_card_h) ? avail_h : max_card_h;
    // Centre the card within the content area, then offset by the render origin.
    g.card_x = ox + (GetNaturalW() - g.card_w) / 2;
    g.card_y = oy + (GetNaturalH() - g.card_h) / 2;
    ...
    // v1.10.3.10.5: ABOUT_ROW_H 38 → 28 so 14 rows + 4 section headings fit
    // inside the 780×800 natural canvas's auto-sized card.
    g.row_h  = ABOUT_ROW_H;
...
void QdAboutElement::RenderRows(SDL_Renderer *r, const AboutGeo &geo) {
    ...
    //   Rows 12-15  → section 3 "Power" (12-13 NFC/USB3, 14-15 SD/Wi-Fi — Sphaira sprint 1)
    //
...
void QdAboutElement::OnRender(pu::ui::render::Renderer::Ref &/*drawer*/,
    ...
    // Compute runtime geometry from natural dimensions and render origin.
    const AboutGeo geo = ComputeGeo(x, y);

    // Full-content dark scrim.
    SDL_SetRenderDrawBlendMode(r, SDL_BLENDMODE_BLEND);
    SDL_SetRenderDrawColor(r,
        theme_.desktop_bg.r,
        theme_.desktop_bg.g,
        theme_.desktop_bg.b,
        240);
    SDL_Rect screen = { x, y, GetNaturalW(), GetNaturalH() };
