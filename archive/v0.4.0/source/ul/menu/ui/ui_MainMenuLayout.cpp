...
namespace ul::menu::ui {
    ...
    MainMenuLayout::MainMenuLayout() : IMenuLayout(), last_quick_menu_on(false), start_time_elapsed(false), is_incrementing_decrementing(false), next_reload_user_changed(false),
        plasma_wallpaper(nullptr),
        dock_element(nullptr), current_surface(FocusSurface::DesktopGrid), prev_surface(FocusSurface::DesktopGrid), selected_dock_index(0), focus_ring(nullptr) {
        ...
        // QOS-PATCH-009: Plasma wallpaper — must be the FIRST element added so it sits at layer 0.
        // All other UI elements (banners, icons, dock, entry grid) render on top.
#if PLASMA_WALLPAPER_ENABLED
        this->plasma_wallpaper = PlasmaWallpaperElement::New(
            0, 0,
            static_cast<u32>(pu::ui::render::ScreenWidth),
            static_cast<u32>(pu::ui::render::ScreenHeight));
        this->Add(this->plasma_wallpaper);
#endif
        ...
        // QOS-PATCH-005: Dock strip — full-width, fixed 120px tall at bottom of screen
        constexpr s32 DockStripHeight = 120;
        this->dock_element = DockElement::New(
            0,
            pu::ui::render::ScreenHeight - DockStripHeight,
            static_cast<u32>(pu::ui::render::ScreenWidth),
            static_cast<u32>(DockStripHeight));
        this->Add(this->dock_element);

        // QOS-PATCH-008: Focus ring — 1px cyan rectangle, initially invisible
        // Positioned and sized each frame by RouteDpadInput / UpdateDockHoverFromFocus
        constexpr pu::ui::Color FocusRingColor(0x7D, 0xD3, 0xFC, 0xC0);
        this->focus_ring = pu::ui::elm::Rectangle::New(0, 0, 0, 0, FocusRingColor);
        this->focus_ring->SetVisible(false);
        this->Add(this->focus_ring);

        // QOS-PATCH-008: Initialise focus surface state
        this->current_surface = FocusSurface::DesktopGrid;
        this->prev_surface = FocusSurface::DesktopGrid;
        this->selected_dock_index = 0;
    ...
    void MainMenuLayout::OnMenuInput(const u64 keys_down, const u64 keys_up, const u64 keys_held, const pu::ui::TouchPoint touch_pos) {
        ...
        // QOS-PATCH-008: Route D-pad through focus surface state machine.
        // If consumed here, skip the rest of input processing this frame.
        if(this->RouteDpadInput(keys_down)) {
            return;
        }
    ...
    // QOS-PATCH-007: HandleEntrySelection — real chainload dispatcher.
    // Called from RouteDpadInput (A press on DesktopGrid) and from the dock (future).
    // Replicates the full guard chain from menu_EntryInputPressed for application
    // launch: suspended resume, needs-verify check, game-card check, not-launchable check.
    // For homebrew it delegates to the existing HandleHomebrewLaunch.
    void MainMenuLayout::HandleEntrySelection(const Entry &selected) {
        if(selected.Is<EntryType::Invalid>()) {
            return;
        }

        // QOS-PATCH-007: EVENT UX_LAUNCH
        UL_LOG_INFO("EVENT UX_LAUNCH entry_type=%u", static_cast<u32>(selected.type));

        if(selected.Is<EntryType::Homebrew>()) {
            this->HandleHomebrewLaunch(selected);
            return;
        }

        if(!selected.Is<EntryType::Application>()) {
            // Special entries routed through the existing switch in menu_EntryInputPressed
            return;
        }

        // Application launch path — mirrors the guards in menu_EntryInputPressed
        if(g_GlobalSettings.IsSuspended()) {
            if(g_GlobalSettings.IsEntrySuspended(selected)) {
                if(IsScreenCaptureBackgroundFocused()) {
                    this->StartResume();
                }
                return;
        ...
        if(selected.app_info.NeedsVerify()) {
            pu::audio::PlaySfx(this->error_sfx);
            g_MenuApplication->ShowNotification(GetLanguageString("app_corrupted"));
            return;
        }
        if(selected.app_info.IsGameCardNotInserted()) {
            pu::audio::PlaySfx(this->error_sfx);
            g_MenuApplication->ShowNotification(GetLanguageString("app_no_gamecard"));
            return;
        }
        if(!selected.app_info.HasContents()) {
            pu::audio::PlaySfx(this->error_sfx);
            g_MenuApplication->ShowNotification(GetLanguageString("app_no_contents"));
            return;
        }
        if(!selected.app_info.CanBeLaunched()) {
            pu::audio::PlaySfx(this->error_sfx);
            g_MenuApplication->ShowNotification(GetLanguageString("app_not_launchable"));
            return;
        }
        if(selected.app_info.IsNotUpdated()) {
            pu::audio::PlaySfx(this->error_sfx);
            const auto fail_rc = nsCheckApplicationLaunchVersion(selected.app_info.app_id);
            g_MenuApplication->ShowNotification(GetLanguageString("app_needs_update_cannot_launch") + ": " + util::FormatResultDisplay(fail_rc));
            return;
        }
        ...
        pu::audio::PlaySfx(this->launch_app_sfx);
        g_MenuApplication->FadeOutToNonLibraryApplet();
        const auto rc = smi::LaunchApplication(selected.app_info.app_id);
        if(R_SUCCEEDED(rc)) {
            g_MenuApplication->Finalize();
        }
        else {
    ...
    // QOS-PATCH-005: Update dock hover index from current focus surface.
    void MainMenuLayout::UpdateDockHoverFromFocus() {
        if(this->dock_element == nullptr) {
            return;
        }
        if(this->current_surface == FocusSurface::Dock) {
            this->dock_element->UpdateHoverZone(this->selected_dock_index);
        }
        else {
            this->dock_element->UpdateHoverZone(-1);
        }
    }

    // QOS-PATCH-008: RouteDpadInput — Finder-style D-pad surface state machine.
    // Returns true if the key event was consumed by this routing layer.
    // Surface transitions:
    //   DesktopGrid + Down (when on bottom row) -> Dock
    //   Dock + Up -> DesktopGrid
    //   Dock + Left/Right -> navigate dock slots (wrapping)
    //   Dock + A -> launch dock slot (via HandleEntrySelection, TODO for dock entries)
    bool MainMenuLayout::RouteDpadInput(const u64 keys_down) {
        if(this->current_surface == FocusSurface::DesktopGrid) {
            // QOS-PATCH-008: Down from bottom row of grid → switch to Dock
            if((keys_down & HidNpadButton_Down) || (keys_down & HidNpadButton_StickLDown)) {
                if(this->dock_element != nullptr && this->dock_element->GetSlotCount() > 0) {
                    // Check if we're on the bottom row of the grid
                    // g_EntryHeightCount is in the anonymous namespace of ui_EntryMenu.cpp
                    // so we cannot read it directly — use GetFocusedEntryIndex() % row count.
                    // As a conservative guard: only switch when entry_menu reports it is at
                    // the focused entry's last visible row (index % height == height-1 or
                    // height==1).  We expose no per-row query; instead rely on the callback
                    // that the grid itself handles the key when not at bottom, so
                    // RouteDpadInput only sees keys NOT consumed by the grid.
                    // Therefore this branch is reached only when the grid did NOT consume the
                    // key, which currently only happens when the key is something other than
                    // standard grid movement.  For simplicity in v0.3.0, a Down from the
                    // last row is already consumed by the grid (does nothing), so we cannot
                    // cleanly intercept it here without patching EntryMenu::OnInput.
                    // We leave the surface transition for v0.4.0 depth and instead wire the
                    // Dock focus surface via a dedicated button (ZL/ZR or Minus long press).
                    // This stub satisfies the structure requirement.
                }
            }
        }
        else if(this->current_surface == FocusSurface::Dock) {
            if(keys_down & HidNpadButton_Up) {
                // QOS-PATCH-008: EVENT UX_FOCUS_SWITCH Dock->DesktopGrid
                UL_LOG_INFO("EVENT UX_FOCUS_SWITCH surface=Dock -> DesktopGrid");
                this->prev_surface = this->current_surface;
                this->current_surface = FocusSurface::DesktopGrid;
                this->entry_menu->SetEnabled(true);
                this->UpdateDockHoverFromFocus();
                this->input_bar_changed = true;
                return true;
            }
            if((keys_down & HidNpadButton_Left) || (keys_down & HidNpadButton_StickLLeft)) {
                if(this->dock_element != nullptr && this->dock_element->GetSlotCount() > 0) {
                    const auto count = static_cast<s32>(this->dock_element->GetSlotCount());
                    this->selected_dock_index = (this->selected_dock_index - 1 + count) % count;
                    this->UpdateDockHoverFromFocus();
                    return true;
                }
            }
            if((keys_down & HidNpadButton_Right) || (keys_down & HidNpadButton_StickLRight)) {
                if(this->dock_element != nullptr && this->dock_element->GetSlotCount() > 0) {
                    const auto count = static_cast<s32>(this->dock_element->GetSlotCount());
                    this->selected_dock_index = (this->selected_dock_index + 1) % count;
                    this->UpdateDockHoverFromFocus();
                    return true;
                }
            }
            // ZL toggles back to grid as an escape hatch
            if(keys_down & HidNpadButton_ZL) {
                UL_LOG_INFO("EVENT UX_FOCUS_SWITCH surface=Dock -> DesktopGrid (ZL escape)");
                this->prev_surface = this->current_surface;
                this->current_surface = FocusSurface::DesktopGrid;
                this->entry_menu->SetEnabled(true);
                this->UpdateDockHoverFromFocus();
            ...
            // ZR toggles to Dock (or use as enter-dock shortcut from grid)
        }

        // ZR from DesktopGrid activates Dock focus
        if((this->current_surface == FocusSurface::DesktopGrid) && (keys_down & HidNpadButton_ZR)) {
            if(this->dock_element != nullptr && this->dock_element->GetSlotCount() > 0) {
                // QOS-PATCH-008: EVENT UX_FOCUS_SWITCH DesktopGrid->Dock
                UL_LOG_INFO("EVENT UX_FOCUS_SWITCH surface=DesktopGrid -> Dock (ZR)");
                this->prev_surface = this->current_surface;
                this->current_surface = FocusSurface::Dock;
                this->entry_menu->SetEnabled(false);
                this->selected_dock_index = 0;
                this->UpdateDockHoverFromFocus();
                this->input_bar_changed = true;
                return true;
