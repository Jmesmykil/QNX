...
namespace ul::menu::ui {
    ...
    void InitializeScreenCaptures(const smi::MenuStartMode start_mode) {
        ...
        // QOS-PATCH-009: When the plasma wallpaper is active, skip loading the static
        // capture image as the background layer.  The capture image is still used for
        // the suspended-app overlay (it gets composited on top of the plasma layer),
        // so we only skip the initial Background.png / boot-capture load.
#if PLASMA_WALLPAPER_ENABLED
        // Suspended-app capture is still loaded so the resume fade-in works correctly.
        if(start_mode != smi::MenuStartMode::StartupMenuPostBoot && g_GlobalSettings.IsSuspended()) {
