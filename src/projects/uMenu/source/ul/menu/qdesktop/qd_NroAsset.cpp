...
#include <switch.h>                          // NacpStruct, setGetSystemLanguage, SetLanguage
#include <cstring>
#include <cstdio>
#include <cerrno>
#include <cmath>
...
NroIconResult ExtractNroIcon(const char *nro_path) {
    ...
    // M-1 bounds check: reject malformed ASET where icon region extends past EOF.
    if (jpeg_abs_off + static_cast<s64>(icon_size) > file_size) {
        UL_LOG_WARN("ExtractNroIcon: JPEG region out of bounds for '%s' jpeg_abs_off=0x%llX icon_size=0x%llX file_size=0x%llX",
                    nro_path, (unsigned long long)jpeg_abs_off,
                    (unsigned long long)icon_size, (unsigned long long)file_size);
        fsFileClose(&fsf);
        u8 *fb = MakeFallbackIcon(nro_path);
        return NroIconResult{ fb, 64, 64, false };
    }
...
// ── ExtractNroNacp (B1.1) ─────────────────────────────────────────────────
// Read the NACP application name from an NRO's ASET section.
//
// Layout used (all values little-endian, offsets relative to ASET header start):
//   ASET+0x00  magic "ASET" (4 bytes)
//   ASET+0x04  version     (4 bytes, ignored)
//   ASET+0x08  icon_off    (u64)  — descriptor 0
//   ASET+0x10  icon_size   (u64)
//   ASET+0x18  nacp_off    (u64)  — descriptor 1  ← this function
//   ASET+0x20  nacp_size   (u64)
//   ASET+0x28  romfs_off   (u64)  — descriptor 2  (ignored here)
//   ASET+0x30  romfs_size  (u64)
//
// nacp_off and nacp_size are relative to the ASET header start (== nro_size).
// Valid NACP: nacp_size == sizeof(NacpStruct) and nacp_off != 0 and
//             (nacp_off + nacp_size) <= (file_size - nro_size).
//
// Language selection mirrors PickNacpName() in qd_CheatTitleResolver.cpp:
//   1. Current system language (setGetSystemLanguage → setMakeLanguage).
//   2. American English (SetLanguage_ENUS).
//   3. Any non-empty lang[i].name.
//
// Returns empty string on any failure — caller keeps the filename stem.
std::string ExtractNroNacp(const char *nro_path) {
    // ── 1. Open file via kernel IPC ───────────────────────────────────────
    FsFileSystem *sdmc = fsdevGetDeviceFileSystem("sdmc");
    if (!sdmc) {
        UL_LOG_WARN("ExtractNroNacp: fsdevGetDeviceFileSystem(sdmc) null for '%s'", nro_path);
        return {};
    }
    FsFile fsf;
    const Result open_rc = fsFsOpenFile(sdmc, StripSdmcPrefix(nro_path),
                                        FsOpenMode_Read, &fsf);
    if (R_FAILED(open_rc)) {
        UL_LOG_WARN("ExtractNroNacp: fsFsOpenFile failed for '%s' rc=0x%X", nro_path, open_rc);
        return {};
    }

    // ── 2. File size (needed for bounds checks) ───────────────────────────
    s64 file_size = 0;
    Result rc = fsFileGetSize(&fsf, &file_size);
    if (R_FAILED(rc)) {
        UL_LOG_WARN("ExtractNroNacp: fsFileGetSize failed for '%s' rc=0x%X", nro_path, rc);
        fsFileClose(&fsf);
        return {};
    }

    // ── 3. Read and validate NRO header ──────────────────────────────────
    constexpr size_t HDR_SIZE = 0x80;
    u8 hdr[HDR_SIZE];
    u64 bytes_read = 0;
    rc = fsFileRead(&fsf, /*off=*/0, hdr, HDR_SIZE, 0, &bytes_read);
    if (R_FAILED(rc) || bytes_read < HDR_SIZE) {
        UL_LOG_WARN("ExtractNroNacp: NRO header read failed for '%s'", nro_path);
        fsFileClose(&fsf);
        return {};
    }
    if (memcmp(&hdr[0x10], NRO0_MAGIC, 4) != 0) {
        UL_LOG_WARN("ExtractNroNacp: NRO0 magic mismatch for '%s'", nro_path);
        fsFileClose(&fsf);
        return {};
    }
    const u32 nro_size = ReadU32LE(hdr, NRO_OFFSET_NRO_SIZE);
    if (nro_size < HDR_SIZE || static_cast<s64>(nro_size) >= file_size) {
        UL_LOG_WARN("ExtractNroNacp: nro_size invalid for '%s' nro_size=0x%X file_size=0x%llX",
                    nro_path, nro_size, (unsigned long long)file_size);
        fsFileClose(&fsf);
        return {};
    }

    // ── 4. Read ASET header at absolute offset nro_size ──────────────────
    u8 aset_hdr[ASET_HEADER_SIZE];
    bytes_read = 0;
    rc = fsFileRead(&fsf, static_cast<s64>(nro_size),
                    aset_hdr, ASET_HEADER_SIZE, 0, &bytes_read);
    if (R_FAILED(rc) || bytes_read < ASET_HEADER_SIZE) {
        UL_LOG_WARN("ExtractNroNacp: ASET header read failed for '%s'", nro_path);
        fsFileClose(&fsf);
        return {};
    }
    if (memcmp(aset_hdr, ASET_MAGIC, 4) != 0) {
        // NRO has no ASET — normal for stub or stripped NROs; silent fallback.
        fsFileClose(&fsf);
        return {};
    }

    // ── 5. Read NACP descriptor at ASET+0x18 ─────────────────────────────
    const u64 nacp_off  = ReadU64LE(aset_hdr, 0x18);
    const u64 nacp_size = ReadU64LE(aset_hdr, 0x20);

    // NACP descriptor with offset==0 and size==0 means no NACP.
    if (nacp_off == 0 && nacp_size == 0) {
        fsFileClose(&fsf);
        return {};
    }

    // Sanity: nacp_size must match NacpStruct exactly.
    if (nacp_size != sizeof(NacpStruct)) {
        UL_LOG_WARN("ExtractNroNacp: unexpected nacp_size for '%s' got=0x%llX expected=0x%zX",
                    nro_path, (unsigned long long)nacp_size, sizeof(NacpStruct));
        fsFileClose(&fsf);
        return {};
    }

    // Bounds: nacp_off + nacp_size must not exceed (file_size - nro_size).
    // nacp_off is relative to ASET header start (which is at nro_size).
    const u64 aset_region = static_cast<u64>(file_size) - static_cast<u64>(nro_size);
    if (nacp_off > aset_region || nacp_size > aset_region - nacp_off) {
        UL_LOG_WARN("ExtractNroNacp: NACP bounds exceed file for '%s' nacp_off=0x%llX nacp_size=0x%llX aset_region=0x%llX",
                    nro_path,
                    (unsigned long long)nacp_off,
                    (unsigned long long)nacp_size,
                    (unsigned long long)aset_region);
        fsFileClose(&fsf);
        return {};
    }

    // ── 6. Read NacpStruct at absolute offset (nro_size + nacp_off) ──────
    NacpStruct nacp;
    bytes_read = 0;
    const s64 nacp_abs_off = static_cast<s64>(nro_size) + static_cast<s64>(nacp_off);
    rc = fsFileRead(&fsf, nacp_abs_off, &nacp, sizeof(NacpStruct), 0, &bytes_read);
    fsFileClose(&fsf);
    if (R_FAILED(rc) || bytes_read < sizeof(NacpStruct)) {
        UL_LOG_WARN("ExtractNroNacp: NacpStruct read failed for '%s' rc=0x%X got=%llu need=%zu",
                    nro_path, rc, (unsigned long long)bytes_read, sizeof(NacpStruct));
        return {};
    }

    // ── 7. Pick display name — mirrors PickNacpName() in qd_CheatTitleResolver.cpp ──
    // Language selection order:
    //   a. Current system language.
    //   b. American English (SetLanguage_ENUS).
    //   c. Any non-empty language entry.
    {
        u64 lang_code = 0;
        ...
        if (lang < SetLanguage_Total) {
            ...
            if (le.name[0] != '\0') {
                std::string result(le.name);
                if (result.size() > 63) { result.resize(63); }
                UL_LOG_INFO("ExtractNroNacp: NACP name='%s' for '%s'", result.c_str(), nro_path);
                return result;
            }
        }
        // b. American English.
        {
            ...
            if (le.name[0] != '\0') {
                std::string result(le.name);
                if (result.size() > 63) { result.resize(63); }
                UL_LOG_INFO("ExtractNroNacp: NACP name(en-US)='%s' for '%s'", result.c_str(), nro_path);
                return result;
            }
        }
        // c. Any non-empty entry.
        for (size_t i = 0; i < 16; ++i) {
            const NacpLanguageEntry &le = nacp.lang[i];
            if (le.name[0] != '\0') {
                std::string result(le.name);
                if (result.size() > 63) { result.resize(63); }
                UL_LOG_INFO("ExtractNroNacp: NACP name(lang[%zu])='%s' for '%s'",
                            i, result.c_str(), nro_path);
                return result;
            }
        }
    }

    // All lang entries empty — treat as no NACP name.
    UL_LOG_WARN("ExtractNroNacp: all NACP lang entries empty for '%s'", nro_path);
    return {};
}
