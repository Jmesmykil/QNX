...
void QdTaskManager::Open(SDL_Renderer *r) {
    ...
    // W15-A P1 FIX: do NOT call nsInitialize/nsExit here.  __appInit (main.cpp)
    // already initialized the ns session at process start; calling nsInitialize
    // here increments the libnx refcount, and our matching nsExit on close
    // decrements it.  If TaskManager closes while QdNsIconCache or
    // MenuControlEntryLoadFunction is mid-IPC, the refcount can hit zero and
    // invalidate the session — same bug pattern as W5-TRANSITIONS #4 that
    // bit QdNsIconCache (already fixed there).  Reuse the process-lifetime
    // session; leave ns_open_ false so the symmetric exit at Close() is a no-op.
