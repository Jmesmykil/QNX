...
namespace pu::ui::render {
    ...
    // v0.6.12-debug: lightweight checkpoint macro — appends to sdmc:/switch/qos-menu-init.log
    #define PU_INIT_LOG(msg) { \
        FILE *_f = fopen("sdmc:/switch/qos-menu-init.log", "a"); \
        if(_f) { fputs("[PUINIT] " msg "\n", _f); fflush(_f); fclose(_f); } \
    }

    Result Renderer::Initialize() {
        if(!this->initialized) {
            PU_INIT_LOG("Initialize: entered");
            ...
            PU_INIT_LOG("Initialize: before SDL_Init");
            if(SDL_Init(this->init_opts.sdl_flags) != 0) {
                return ResultSdlInitFailed;
            }

            PU_INIT_LOG("Initialize: after SDL_Init, before SDL_CreateWindow");
            ...
            PU_INIT_LOG("Initialize: after SDL_CreateWindow, before SDL_CreateRenderer");
            {
                int num_drivers = SDL_GetNumRenderDrivers();
                FILE *_df = fopen("sdmc:/switch/qos-menu-init.log", "a");
                if(_df) {
                    fprintf(_df, "[PUINIT] SDL_GetNumRenderDrivers=%d\n", num_drivers);
                    for(int i = 0; i < num_drivers; i++) {
                        SDL_RendererInfo info;
                        if(SDL_GetRenderDriverInfo(i, &info) == 0) {
                            fprintf(_df, "[PUINIT]   driver[%d]: name='%s' flags=0x%08X num_fmts=%u max_w=%d max_h=%d\n",
                                    i, info.name, (unsigned)info.flags,
                ...
                // Try forcing SDL to pick the "software" render driver by name
                SDL_SetHint(SDL_HINT_RENDER_DRIVER, "software");

                FILE *_df2 = fopen("sdmc:/switch/qos-menu-init.log", "a");
                if(_df2) {
                    fputs("[PUINIT] HINT_RENDER_DRIVER=software set, calling SDL_CreateRenderer\n", _df2);
                    fflush(_df2);
                    fclose(_df2);
                }
            }
...

            {
                ...
                if(_rf) {
                    if(g_Renderer == nullptr) {
                        fprintf(_rf, "[PUINIT] SDL_CreateRenderer returned NULL: %s\n", SDL_GetError());
                    } else {
                        SDL_RendererInfo info;
                        if(SDL_GetRendererInfo(g_Renderer, &info) == 0) {
                            fprintf(_rf, "[PUINIT] Initialize: after SDL_CreateRenderer — picked driver '%s' flags=0x%08X\n",
                                    info.name, (unsigned)info.flags);
                        } else {
                            fputs("[PUINIT] Initialize: after SDL_CreateRenderer (GetRendererInfo failed)\n", _rf);
                        }
                    }
                    fflush(_rf);
                    fclose(_rf);
            ...
            PU_INIT_LOG("Initialize: complete");
        }
