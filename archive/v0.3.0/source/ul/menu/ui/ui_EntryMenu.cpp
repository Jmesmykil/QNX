...
// Q OS fork patch v0.2.1 (QOS-PATCH-002): paging guard — SwipeToNextPage/SwipeToPreviousPage
// now clamp to actual entry count so scrolling never wraps past the last app icon.
// See INTEGRATION-SPEC.md §2, ROADMAP.md v0.2.1 entry.
// Q OS fork patch v0.3.0 (QOS-PATCH-006): D-pad grid wrap — Left at column 0 wraps to last
// real column; Right at last real column wraps back to column 0.
// Emits EVENT UX_GRID_WRAP via UL_LOG_INFO on each wrap transition.
...
namespace ul::menu::ui {
    ...
    void EntryMenu::SwipeToNextPage() {
        // QOS-PATCH-002: clamp so we never scroll past the last real entry.
        // Upstream had no guard here; on a small library the cursor would land
        // on an empty cell and the selection highlight would vanish off-screen.
        const auto old_entry_idx = this->cur_entry_idx;
        const u32 total_entries = static_cast<u32>(this->cur_entries.size());
        const u32 page_size = this->entry_width_count * g_EntryHeightCount;
        if(total_entries == 0 || this->cur_entry_idx + page_size >= total_entries) {
            // Already on last page — no-op (matches mock-desktop-gui paging model).
            return;
        }
        this->base_entry_idx_x += this->entry_width_count;
        this->cur_entry_idx += page_size;
        this->NotifyFocusedEntryChanged(old_entry_idx);
    }
    ...
    void EntryMenu::OnInput(const u64 keys_down, const u64 keys_up, const u64 keys_held, const pu::ui::TouchPoint touch_pos) {
        ...
        if((keys_down & HidNpadButton_Up) || (keys_held & HidNpadButton_StickLUp) || (keys_held & HidNpadButton_StickRUp)) {
            ...
            // QOS-PATCH-006: Left at column 0 of the first page — wrap to last real column
            else if((rel_entry_idx_x == 0) && (this->base_entry_idx_x == 0)) {
                if(this->IsNotInSwipe() && !this->cur_entries.empty()) {
                    const auto total_entries = static_cast<u32>(this->cur_entries.size());
                    const auto last_col_idx_x = (total_entries - 1) / g_EntryHeightCount;
                    const auto last_col_base = (last_col_idx_x / this->entry_width_count) * this->entry_width_count;
                    const auto cur_idx_y = this->cur_entry_idx % g_EntryHeightCount;
                    const auto wrap_target_idx = last_col_idx_x * g_EntryHeightCount + cur_idx_y;
                    const auto clamped_target = std::min(wrap_target_idx, total_entries - 1);
                    // QOS-PATCH-006: EVENT UX_GRID_WRAP
                    UL_LOG_INFO("EVENT UX_GRID_WRAP direction=left col=%u -> col=%u", 0u, last_col_idx_x);
                    const auto old_entry_idx = this->cur_entry_idx;
                    this->base_entry_idx_x = last_col_base;
                    this->cur_entry_idx = clamped_target;
                    this->cur_entry_change_started_cb();
                    this->NotifyFocusedEntryChanged(static_cast<s32>(old_entry_idx));
                }
            }
        }
            ...
            if((rel_entry_idx_x + 1) < this->entry_width_count) {
                ...
                if(this->IsNotInSwipe()) {
                    // QOS-PATCH-006: Check if the next column has any real entries.
                    // The next column's absolute index is (base_entry_idx_x + entry_width_count) * g_EntryHeightCount.
                    // If that index is >= real entry count, wrap to column 0 instead of swiping right.
                    const auto next_col_start = (this->base_entry_idx_x + this->entry_width_count) * g_EntryHeightCount;
                    const auto total_entries = static_cast<u32>(this->cur_entries.size());
                    if(next_col_start >= total_entries) {
                        // QOS-PATCH-006: EVENT UX_GRID_WRAP — right wrap to column 0
                        const auto cur_idx_x = this->cur_entry_idx / g_EntryHeightCount;
                        UL_LOG_INFO("EVENT UX_GRID_WRAP direction=right col=%u -> col=0", cur_idx_x);
                        this->SwipeRewind();
                        this->entries_base_swipe_neg_offset = 0;
                        this->entries_base_swipe_neg_offset_incr.StartFromZero(EntriesSwipeRewindIncrementSteps, this->entry_size + this->entry_h_margin);
