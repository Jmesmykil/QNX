...
namespace ul::menu::ui {
    ...
    void InitializeScreenCaptures(const smi::MenuStartMode start_mode) {
        ...
        if(start_mode != smi::MenuStartMode::StartupMenuPostBoot) {
            // v0.6.6: lazy/conditional allocation — only allocate the 3.67 MB capture
            // buffer when we actually need it (i.e. when entering from a non-boot path).
            // Use nothrow so a low-memory situation degrades gracefully instead of
            // aborting: g_ScreenCaptureBackground remains an empty RawRgbaImage,
            // HasScreenCaptureBackground() returns false, and the layout falls back to
            // the static wallpaper with no visible crash.
            auto capture_buf = new(std::nothrow) u8[CaptureBufferSize]();
            if(capture_buf == nullptr) {
                UL_LOG_WARN("InitializeScreenCaptures: failed to allocate %zu-byte capture buffer (nothrow) — skipping screen capture, using static wallpaper", CaptureBufferSize);
