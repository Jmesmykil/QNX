...
# Target 1: /hbmenu.nro at SD root (canonical SwitchBrew install layout)
HBMENU_NRO="$SD_PATH/hbmenu.nro"

# Target 1b: /switch/hbmenu.nro (alternative layout — many CFW guides put the
# launcher inside /switch/ alongside other NROs).  Added 2026-05-18 after the
# Q OS HW test found the user's hbmenu at this path instead of the root.
HBMENU_NRO_SWITCH="$SD_PATH/switch/hbmenu.nro"
...
if [[ -f "$HBMENU_NRO_SWITCH" ]]; then
    SZ=$(human_size "$HBMENU_NRO_SWITCH")
    RAW=$(bytes_for "$HBMENU_NRO_SWITCH")
    printf '  [REMOVE]  %s  (%s)\n' "$HBMENU_NRO_SWITCH" "$SZ"
    TOTAL_BYTES=$(( TOTAL_BYTES + RAW ))
    FOUND_ANY=1
else
    printf '  [skip]    %s  (not present)\n' "$HBMENU_NRO_SWITCH"
...
[[ -f "$HBMENU_NRO_SWITCH" ]]  && remove_file "$HBMENU_NRO_SWITCH"
