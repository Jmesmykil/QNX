...
namespace pu::ui::render {
    ...
    Result Renderer::Initialize() {
        if(!this->initialized) {
            ...
            // WIN-1 (Q OS perf): batch draw calls into the GPU command stream
            // instead of flushing each synchronously — major win on Tegra X1 under
            // load (100 windows). Must be set BEFORE SDL_CreateRenderer.
