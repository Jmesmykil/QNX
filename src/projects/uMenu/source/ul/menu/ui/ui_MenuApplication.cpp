...
#include <ul/menu/qdesktop/qd_Audio.hpp>   // QdAudio::Initialize / Finalize
#include <atomic>
...
namespace ul::menu::ui {
        ...
        switch(type) {
            ...
            case MenuType::Vault:
                if(this->vault_lyt == nullptr) {
                    const qdesktop::QdTheme &qdt = qdesktop::g_QdTheme;  // v2.4.0 Phase B: live palette
            ...
            case MenuType::QSettings:
                if(this->qsettings_lyt_ == nullptr) {
                    const qdesktop::QdTheme &qdt_s = qdesktop::g_QdTheme;  // v2.4.0 Phase B: live palette
            ...
            case MenuType::QLockscreen:
                if(this->qlockscreen_lyt_ == nullptr) {
                    const qdesktop::QdTheme &qdt_l = qdesktop::g_QdTheme;  // v2.4.0 Phase B: live palette
    ...
    void MenuApplication::OnLoad() {
        ...
        // W5-TRANSITIONS #1: store handle so DisposeAllSfx can free the Mix_Chunk.
        {
            static bool g_boot_chime_sfx_played = false;
            if(!g_boot_chime_sfx_played) {
                g_boot_chime_sfx_played = true;
                this->boot_chime_sfx_ = pu::audio::LoadSfx(TryGetActiveThemeResource("sound/Launchpad/BootChime.wav"));
                if(this->boot_chime_sfx_) {
                    pu::audio::PlaySfx(this->boot_chime_sfx_);
                }
            }
        }
        ...
        // BOOT-TRACE: OnLoad returned — the render loop (Show/ShowWithFadeIn)
        // is about to start.  Register a one-shot render callback that fires on
        // the very first frame and writes the FIRST_RENDER marker, proving the
        // compositor is presenting frames.
        {
            // Use a static flag so we only fire this once per process lifetime.
            static std::atomic<bool> s_first_render_traced { false };
            this->AddRenderCallback([]() {
                if (!s_first_render_traced.exchange(true, std::memory_order_acq_rel)) {
                    // Write directly via raw fopen so we don't depend on any
                    // state that might still be settling.
                    FILE *f = fopen("sdmc:/ulaunch/umenu_boot_trace.log", "ab");
                    if (f) {
                        char buf[128];
                        int n = ::snprintf(buf, sizeof(buf),
                            "[BOOT_TRACE tick=%lu] FIRST OnRender frame — compositor presenting\n",
                            static_cast<unsigned long>(armGetSystemTick()));
                        if (n > 0) { fwrite(buf, static_cast<size_t>(n), 1, f); fflush(f); }
                        fclose(f);
                    }
                }
            });
    ...
    void MenuApplication::ShowNotification(const std::string &text, const u64 timeout) {
        qdesktop::QdAudio::Play(qdesktop::DesktopSfxEvent::ToastAppear);
        this->EndOverlay();
    ...
    void MenuApplication::StartPlayBgm() {
        ...
        if(bgm.bgm != nullptr) {
            ...
            if(this->loaded_menu == MenuType::Startup) {
                qdesktop::QdAudio::Play(qdesktop::DesktopSfxEvent::LoginBgmFadeIn);
            }
        }
    }
    ...
    void MenuApplication::LoadBgmSfxForCreatedMenus() {
        ...
        // Phase 2: load the global desktop SFX coordinator.  Called here so
        // the table is populated after the active-theme path is resolved and
        // after Phase-1 per-layout LoadSfx() calls complete.  Re-entrant:
        // Initialize() calls Finalize() internally if already loaded (safe on
        // theme-switch).
        qdesktop::QdAudio::Initialize();
    }
    ...
    void MenuApplication::DisposeAllSfx() {
        ...
        // W5-TRANSITIONS #1: free the boot chime Mix_Chunk stored in ctor.
        if(this->boot_chime_sfx_ != nullptr) {
            pu::audio::DestroySfx(this->boot_chime_sfx_);
        }
        // Phase 2: release the global desktop SFX coordinator table.
        qdesktop::QdAudio::Finalize();
    }
    ...
    int MenuApplication::DisplayDialog(const std::string &title, const std::string &content, const std::vector<std::string> &opts, const bool use_last_opt_as_cancel, pu::sdl2::TextureHandle::Ref icon) {
        qdesktop::QdAudio::Play(qdesktop::DesktopSfxEvent::DialogOpen);
        const int result = this->CreateShowDialog(title, content, opts, use_last_opt_as_cancel, icon, [&](pu::ui::Dialog::Ref &dialog) {
        ...
        qdesktop::QdAudio::Play(qdesktop::DesktopSfxEvent::DialogClose);
        return result;
    }
