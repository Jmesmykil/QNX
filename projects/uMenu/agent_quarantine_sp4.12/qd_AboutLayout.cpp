...
void QdAboutElement::Refresh() {
    ...
    {
        AppletOperationMode op_mode = AppletOperationMode_Handheld;
        const Result rc = appletGetOperationMode(&op_mode);
        if (R_FAILED(rc)) {
            snprintf(rows_[5].value, sizeof(rows_[5].value), "n/a");
        } else {
        ...
        if (R_FAILED(rc)) {
            ...
            switch (charger) {
                case PsmChargerType_Unconnected:
                    ...
                    break;
                case PsmChargerType_RegularCharger:
                    snprintf(rows_[7].value, sizeof(rows_[7].value), "Regular charger");
                    break;
                case PsmChargerType_LowPowerCharger:
